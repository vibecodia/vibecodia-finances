...
export const useCategories = () => {
  ...
  const [expenseCategories, setExpenseCategories] = useLocalStorage<any[]>(
    "manageable_expense_categories",
    EXPENSE_CATEGORIES,
  );
  const [incomeCategories, setIncomeCategories] = useLocalStorage<any[]>(
    "manageable_income_categories",
    INCOME_CATEGORIES,
  );
  ...
  useEffect(() => {
    ...
    const hasExpenseChange =
      normalizedExpense.length !== expenseCategories.length ||
      normalizedExpense.some((cat, i) => cat !== expenseCategories[i]);
    const hasIncomeChange =
      normalizedIncome.length !== incomeCategories.length ||
      normalizedIncome.some((cat, i) => cat !== incomeCategories[i]);
  ...
  const removeCategory = (
    ...
    const isInUse = transactions.some(
      (t) => t.type === type && t.category === categoryToRemove,
    );
    ...
    if (type === "expense") {
      setExpenseCategories(
        expenseCategories.filter((cat: string) => cat !== categoryToRemove),
      );
    } else {
      setIncomeCategories(
        incomeCategories.filter((cat: string) => cat !== categoryToRemove),
      );
    }
