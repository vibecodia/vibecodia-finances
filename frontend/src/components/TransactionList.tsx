...
const TransactionList: React.FC<TransactionListProps> = ({
  ...
  const [transactionToReplicate, setTransactionToReplicate] =
    useState<Transaction | null>(null);
  ...
  const [animatedTransactionId, setAnimatedTransactionId] = useState<
    string | null
  >(null);
  ...
  const allMonthTransactions = filterTransactionsByMonth(
    transactions
      ...
      .filter((t) => {
        ...
        return (
          t.paymentMethod &&
          paymentMethodFilter.includes(formatPaymentMethod(t.paymentMethod))
        );
      })
      .filter((t) => {
        if (searchTerm) {
  ...
  const baseActiveTransactions = allMonthTransactions.filter(
    (t) => t.status !== "deleted",
  );
  const baseDeletedTransactions = allMonthTransactions.filter(
    (t) => t.status === "deleted",
  );
  ...
  useEffect(() => {
    ...
    if (highlightId) {
      const transaction = transactions.find(
        (t) => t.id === highlightId || t._id === highlightId,
      );
      if (transaction) {
        // Set month to transaction month if it's different
        const transactionDate = parseLocalDate(transaction.date);
        if (
          !isSameDay(startOfMonth(currentMonth), startOfMonth(transactionDate))
        ) {
  ...
  useEffect(() => {
    if (searchTerm !== undefined) {
      // Only trigger if searchTerm is a controlled prop
      setIsSearching(true);
  ...
  useEffect(() => {
    ...
    if (
      startDateFilter &&
      endDateFilter &&
      (!isSameDay(startDateFilter, start) || !isSameDay(endDateFilter, end))
    ) {
  ...
  const [transactionToReactivate, setTransactionToReactivate] = useState<
    string | null
  >(null);
  ...
  const transactionsForDisplay = showDeleted
    ? deletedTransactions
    : activeTransactions;
      ...
      if (editingTransaction) {
        ...
        if (
          ...
          const goal = savingsGoals.find(
            (g) => (g.id || g._id) === newTransaction.savingsGoalId,
          );
  ...
  const handleReactivateConfirm = () => {
    if (transactionToReactivate) {
      onUpdate(transactionToReactivate, {
        status: "active",
        deletedAt: undefined,
      });
    }
  ...
  const handlePressStart = (
    ...
    longPressTimeoutRef.current = setTimeout(() => {
      ...
      pressTimer.current = setTimeout(() => {
        if (countdownTimer.current) {
          // Ensure countdown completed naturally
          clearInterval(countdownTimer.current);
          countdownTimer.current = null;
        }
  ...
  const handlePressEnd = () => {
    if (longPressTimeoutRef.current) {
      // Clear the initial long press timeout
      clearTimeout(longPressTimeoutRef.current);
  ...
  const handleUpdatePaymentStatusAndAnimate = async (
    id: string,
    isPaid: boolean,
  ) => {
  ...
  return (
    <div className="space-y-6">
      {apporteMessage && (
        <Card
          className="p-4 border-2 animate-in slide-in-from-top-2 duration-300"
          ...
          <div className="flex items-center flex-wrap gap-x-4 gap-y-2">
            ...
            {activeTransactions.length > 0 && (
              <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                ...
                {activeTransactions.length === 1 ? "item" : "itens"}
              </span>
            )}
            {deletedTransactions.length > 0 && (
              ...
              >
                ...
                {deletedTransactions.length === 1 ? "excluído" : "excluídos"}
                <ChevronDown
                  className={cn(
          ...
          {type === "expense" && (
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
              ...
              <Button
                ...
                variant={
                  paymentMethodFilter.includes("all") ? "primary" : "outline"
                }
                size="sm"
                className="rounded-full text-[10px] uppercase h-8 px-4"
              >
          ...
          {startDateFilter && endDateFilter && (
            <div className="flex items-center gap-3 pb-2">
              ...
              <Button
                ...
                size="sm"
                className={cn(
                  "h-8 text-[10px] uppercase rounded-full",
                  !isDailyFilterActive && "opacity-30",
                )}
            ...
            return (
              <Card
                ...
                className={cn(
                  ...
                  animatedTransactionId === transaction.id &&
                    "animate-pulse-once",
                ...
                style={{
                  borderColor: isDeleted
                    ...
                    : overdue
                      ? theme.primary
                      : !transaction.isPaid && type === "expense"
                        ? theme.accent
                        : theme.cardBorder,
                }}
                onMouseDown={(e) =>
                  !isDeleted && handlePressStart(e, transaction)
                }
                ...
                {activeTransactionId === transaction.id &&
                  countdown !== null && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-xl z-20 backdrop-blur-sm">
                      <span className="text-white text-6xl font-black animate-ping">
                        {countdown}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex-1 min-w-0 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      {!isDeleted && (
                        <button
                          onClick={() =>
                            handleUpdatePaymentStatusAndAnimate(
                              transaction.id,
                              !transaction.isPaid,
                            )
                          }
                        ...
                        {((typeof transaction.notes === "string" &&
                          ...
                          >
                            {expandedNotes[transaction.id] ? (
                              ...
                              <>
                                <ChevronDown className="w-3 h-3 mr-1" /> Ver
                                itens da nota
                              </>
                            )}
                          </Button>
