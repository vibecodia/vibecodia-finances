...
const ITAU_HISTORY: HistoryItem[] = [
  ...
  },
  {
    ...
    situacao: "Paga",
    total: 7717.38,
    saldoDevedor: 588165.6,
    operacao: "Pula parcela (+7.619,83)",
  },
  {
    parcela: 6,
    ...
    situacao: "Paga",
    total: 7817.88,
    saldoDevedor: 595467.88,
    operacao: "Pula parcela (+7.717,38)",
  },
  {
    parcela: 7,
    ...
    situacao: "Paga",
    total: 1797.65,
    saldoDevedor: 595035.32,
    operacao: "Utilização FGTS DAMP III (-72.244,05)",
  },
  {
    parcela: 8,
    ...
    situacao: "Paga",
    total: 1797.2,
    saldoDevedor: 594610.26,
  },
  {
    parcela: 9,
    vencimento: "2026-03-26",
    amortizacao: 1449.19,
    juros: 6137.17,
    seguroMIP: 177.62,
    seguroDFI: 47.82,
  ...
  },
  {
    ...
    situacao: "Projetada",
    total: 1769.21,
    saldoDevedor: 582105.94,
  },
  {
    parcela: 19,
    vencimento: "2027-01-26",
    amortizacao: 1451.64,
    juros: 5997.94,
    seguroMIP: 179.32,
    seguroDFI: 47.91,
    ...
    situacao: "Projetada",
    total: 4475.61,
    saldoDevedor: 580654.3,
  },
  {
    parcela: 20,
    vencimento: "2027-02-26",
    amortizacao: 1451.64,
    juros: 5982.99,
    seguroMIP: 178.87,
    seguroDFI: 47.91,
...
const DEFAULT_LAYOUT: LayoutItem[] = [
  {
    id: "header_summary",
    label: "Resumo do Financiamento",
    collapsed: false,
    number: 1,
  },
  {
    id: "consorcio_summary",
    label: "Resumo Consórcio Porto",
    collapsed: false,
    number: 2,
  },
  {
    id: "itau_consorcio_relation",
    label: "Relação Financiamento Itaú x Consórcio",
    collapsed: false,
    number: 3,
  },
  {
    id: "installments_table",
    label: "Tabela de Parcelas Itaú",
    collapsed: false,
    number: 4,
  },
  {
    id: "consorcio_installments_table",
    label: "Tabela de Parcelas Porto",
    collapsed: true,
    number: 5,
  },
];

const FinanciamentoCasaPlayground: React.FC<
  FinanciamentoCasaPlaygroundProps
> = ({ transactions, theme }) => {
  const [layout, setLayout] = useLocalStorage<LayoutItem[]>(
    "financiamento_playground_layout_v4",
    DEFAULT_LAYOUT,
  );
  ...
  const [overrideTotalParcelas, setOverrideTotalParcelas] = useState<
    number | null
  >(419);
  const [taxaJurosMensal] = useState<number>(0.010303871);
  const [taxaEfetivaAnual] = useState<number>(13.09);
  ...
  const consorcioData = useMemo(() => {
    ...
    const portoTransactions = transactions
      .filter(
        (t) =>
  ...
  const [consorcioModoUso, setConsorcioModoUso] = useState<"total" | "uma">(
    "total",
  );
  const [consorcioIntervaloMeses, setConsorcioIntervaloMeses] = useState(12);
  const [manualContemplacaoDate, setManualContemplacaoDate] = useState<
    string | null
  >(null);
  ...
  const relationData = useMemo(() => {
    ...
    const currentSaldoItau =
      adjustedSacData.find((d) => d.parcela === lastParcelaPaga + 1)
        ?.saldoDevedor || 0;

    // 1. Projeção de Contemplação
    const targetPaidPercent = consorcioMinContemplacao / 100;
    const remainingPercentToTarget = Math.max(
      targetPaidPercent - consorcioData.percentPaidEstimated / 100,
      0,
    );
    const amountToTarget =
      remainingPercentToTarget * consorcioData.valorTotalContrato;
    const monthsToContemplacao = Math.ceil(
      amountToTarget / consorcioData.avgInstallment,
    );

    const projectedContemplacaoDate = manualContemplacaoDate
      ? parseISO(manualContemplacaoDate)
      : addMonths(today, monthsToContemplacao);

    // Parcela do Itaú na data da contemplação
    const itauParcelaAtContemplacao = adjustedSacData.find((d) => {
      const dDate = new Date(d.date);
      return (
        dDate.getMonth() === projectedContemplacaoDate.getMonth() &&
        dDate.getFullYear() === projectedContemplacaoDate.getFullYear()
      );
    });

    const saldoItauAtContemplacao =
      itauParcelaAtContemplacao?.saldoDevedor || 0;
    const parcelaIndexAtContemplacao =
      itauParcelaAtContemplacao?.parcela || totalParcelas;

    // 2. Cenário de Amortização
    const liquidoTotal = consorcioData.totalCreditEstimated;
    const liquidoUso =
      consorcioModoUso === "total" ? liquidoTotal : liquidoTotal / 3;
    ...
    const amortizacaoBaseAtual =
      itauParcelaAtContemplacao?.amortizacao || 1451.64;
    const parcelasRestantesOriginal =
      totalParcelas - parcelaIndexAtContemplacao;
    const novasParcelasRestantes =
      amortizacaoBaseAtual > 0
        ? Math.ceil(novoSaldoItau / amortizacaoBaseAtual)
        : 0;

    const economiaParcelas = Math.max(
      parcelasRestantesOriginal - novasParcelasRestantes,
      0,
    );
    const novaDataQuitacao = addMonths(
    ...
    const economiaJuros =
      liquidoUso * jurosMensal * parcelasRestantesOriginal * 0.5; // Estimativa conservadora (0.5 pelo decréscimo do SAC)
    ...
    const monthsElapsedAtQuitacao = Math.max(
      0,
      (novaDataQuitacao.getFullYear() - consorcioStart.getFullYear()) * 12 +
        (novaDataQuitacao.getMonth() - consorcioStart.getMonth()),
    );

    // O consórcio tem 200 meses no total.
    // Restam = Total - (Meses de uso até a quitação do Itaú)
    const consorcioRestanteNaQuitacao = Math.max(
      200 - monthsElapsedAtQuitacao,
      0,
    );
    ...
    return {
      ...
      mesesAteContemplacao: Math.max(
        0,
        differenceInMonths(projectedContemplacaoDate, today),
      ),
      consorcioRestante: consorcioRestanteNaQuitacao,
      consorcioRestanteValor:
        consorcioRestanteNaQuitacao * consorcioData.avgInstallment,
      sobraCredito,
    };
  }, [
    consorcioData,
    adjustedSacData,
    lastParcelaPaga,
    consorcioMinContemplacao,
    consorcioTaxaAdm,
    manualContemplacaoDate,
    taxaJurosMensal,
    totalParcelas,
    consorcioModoUso,
    consorcioIntervaloMeses,
    today,
  ]);
  ...
  const consorcioInstallmentsData = useMemo(() => {
    ...
    for (let i = 1; i <= 200; i++) {
      ...
      if (i === 1) valorMensal = CONSORCIO_CONFIG.valorPrimeiraParcelaTotal;
        ...
        valorMensal =
          CONSORCIO_CONFIG.valorPrimeiraParcelaTotal +
          (CONSORCIO_CONFIG.valorParcelaAtualTotal -
            CONSORCIO_CONFIG.valorPrimeiraParcelaTotal) *
            progress;
      }

      // Percentual pago do PLANO
      const percentPaidSoFar =
        i <= count
          ? i * pastReductionPerMonth
          : currentPercent + (i - count) * futureReductionPerMonth;

      // Quanto falta para atingir o CRÉDITO LÍQUIDO
      const faltaParaCredito =
        totalCredit * (1 - Math.min(percentPaidSoFar, 1));
  ...
  if (parsedData.length === 0) {
    return (
      <div
        className="flex flex-col items-center justify-center p-20 text-center gap-4"
      ...
      >
        ...
        <div className="max-w-md">
          ...
          <p className="text-muted-foreground text-sm mt-2">
            Não encontramos transações com o padrão "Financiamento casa X/Y" na
  ...
  const lastDate =
    adjustedSacData.length > 0
      ? adjustedSacData[adjustedSacData.length - 1].vencimento
      : "";
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">
                      CARTEIRA HIPOTECARIA | SISTEMA SAC
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <span className="px-3 py-1 bg-accent/10 text-accent text-[10px] font-black rounded-full uppercase">
                      Itaú Real Data
                    </span>
                  </div>
                </div>
                  ...
                  {[
                    {
                      ...
                      value: lastParcelaPaga,
                      icon: <CheckCircle2 className="w-4 h-4" />,
                    },
                    {
                      label: "Taxa Juros (M)",
                      value: `${(taxaJurosMensal * 100).toFixed(4)}%`,
                      icon: <TrendingUp className="w-4 h-4" />,
                    },
                    {
                      label: "Taxa Efetiva (A)",
                      value: `${taxaEfetivaAnual.toFixed(2)}%`,
                      icon: <TrendingUp className="w-4 h-4" />,
                      highlight: true,
                    },
                    {
                      label: "Vencimento Final",
                      value: formatBrazilDate(lastDate),
                      icon: <Calendar className="w-4 h-4" />,
                    },
                  ].map((stat, i) => (
                    <div
                      key={i}
                      className="p-4 rounded-xl border border-border bg-muted/10 flex flex-col gap-1"
                    >
                      ...
                      <div
                        className={`text-sm md:text-base font-black ${stat.highlight ? "text-primary" : "text-foreground"}`}
                      >
                        {stat.value}
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-border">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground uppercase">
                      ...
                      Utilização de FGTS DAMP III aplicada em 06/01/2026: R$
                      72.244,05
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-amber-600">
                      <AlertCircle className="w-4 h-4" />2 Operações "Pula
                      Parcela" realizadas: +R$ 15.337,21 ao saldo devedor
                    </div>
                  </div>
                  <div className="flex items-center justify-end gap-3">
                    <label className="text-[10px] font-black uppercase text-muted-foreground">
                      Ajustar total parcelas:
                    </label>
                    <Input
                      type="number"
                      value={totalParcelas}
                      onChange={(e) =>
                        setOverrideTotalParcelas(parseInt(e.target.value))
                      }
                ...
                {!consorcioData ? (
                  <div className="flex flex-col items-center justify-center p-10 text-center gap-3 text-muted-foreground">
                    ...
                    <p className="text-sm font-bold">
                      Nenhum dado de Consórcio encontrado
                    </p>
                    <p className="text-[10px]">
                      Verifique transações com "Consórcio Porto" na categoria
                      "Patrimônio".
                    </p>
                  </div>
                ) : (
                  <>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
                      <div>
                        <h3 className="text-lg font-black text-accent">
                          Grupo Porto Seguro (3 Cartas)
                        </h3>
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">
                          Início em{" "}
                          {formatBrazilDate(
                            format(consorcioData.firstDate, "yyyy-MM-dd"),
                          )}
                        </p>
                      </div>
                      <div className="flex gap-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        {
                          label: "Parcelas Pagas",
                          value: `${consorcioData.count} / 200`,
                          icon: <CheckCircle2 className="w-4 h-4" />,
                        },
                        {
                          label: "Total Investido",
                          value: formatCurrency(consorcioData.totalPaid),
                          icon: (
                        ...
                        {
                          label: "Parcela Atual (3x)",
                          value: formatCurrency(consorcioData.avgInstallment),
                          icon: <TrendingUp className="w-4 h-4" />,
                        },
                        {
                          label: "Próximo Vencimento",
                          value: formatBrazilDate(
                            format(consorcioData.nextVencimento, "yyyy-MM-dd"),
                          ),
                          icon: <Calendar className="w-4 h-4" />,
                          highlight: consorcioData.isNextPendente,
                          status: consorcioData.isNextPendente
                            ? "Pendente"
                            : "Projetado",
                        },
                      ].map((stat, i) => (
                        ...
                        >
                          ...
                          <div
                            className={`text-sm md:text-base font-black ${stat.highlight ? "text-accent" : "text-foreground"}`}
                          >
                            {stat.value}
                          </div>
                          {stat.status && (
                            <span
                              className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full w-fit ${stat.status === "Pendente" ? "bg-amber-500/20 text-amber-600" : "bg-muted text-muted-foreground"}`}
                            >
                              {stat.status}
                            </span>
                          )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="p-4 rounded-xl border border-border bg-muted/5 space-y-2">
                        ...
                        <div className="text-lg font-black text-primary">
                          {formatCurrency(consorcioData.totalCreditEstimated)}
                        </div>
                        <div className="text-[9px] text-muted-foreground font-bold italic">
                          R${" "}
                          {formatCurrency(
                            consorcioData.detalhePorCarta.credito,
                          )}{" "}
                          por carta
                        </div>
                      </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1 h-2 bg-primary/10 rounded-full relative">
                            <div
                              ...
                              style={{
                                width: `${consorcioData.percentPaidEstimated}%`,
                              }}
                            />
                            {/* Meta Marker */}
                            <div
                      ...
                      <div className="p-4 rounded-xl border border-border bg-muted/5 space-y-2 relative group">
                        ...
                        <div className="text-lg font-black text-destructive">
                          {formatCurrency(consorcioData.totalQuitacao)}
                        </div>
                        <div className="text-[9px] text-muted-foreground font-bold italic">
                          R${" "}
                          {formatCurrency(
                            consorcioData.detalhePorCarta.quitacao,
                          )}{" "}
                          cada (89,91%)
                        </div>
                        <div className="text-[8px] font-bold text-accent bg-accent/5 p-1 rounded border border-accent/10 mt-1">
                          * Inclui taxa de administração de {consorcioTaxaAdm}%
              ...
              <div className="p-6 space-y-8">
                {!relationData ? (
                  <div className="flex flex-col items-center justify-center p-10 text-center gap-3 text-muted-foreground">
                    <AlertCircle className="w-10 h-10" />
                    <p className="text-sm font-bold">
                      Aguardando dados do Consórcio
                    </p>
                    <p className="text-[10px]">
                      A relação depende de dados reais do Consórcio Porto.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="bg-muted/10 p-5 rounded-2xl border border-border space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                          Parâmetros de Simulação Estratégica
                        </h4>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-4">
                        <div className="flex-1 min-w-[140px] space-y-1.5">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setConsorcioTaxaAdm(Number(e.target.value))
                            }
                        ...
                        <div className="flex-1 min-w-[160px] space-y-1.5">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setManualContemplacaoDate(e.target.value || null)
                            }
                            className="font-bold py-2.5"
                          />
                        </div>
                        ...
                        <div className="flex-1 min-w-[200px] space-y-1.5">
                          ...
                          >
                            <option value="total">
                              Crédito Total (3 Cartas)
                            </option>
                            <option value="uma">
                              Faseado (1 Carta p/ vez)
                            </option>
                          </Select>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="text-xs font-black uppercase text-muted-foreground opacity-60">
                            O Gatilho: Contemplação
                          </h4>
                        </div>
                        <div className="p-5 rounded-2xl border-2 border-accent/20 bg-accent/[0.02] space-y-4">
                          <div>
                            <span className="text-[10px] font-black uppercase text-muted-foreground opacity-50 block mb-1">
                              Data Estimada do Aporte
                            </span>
                            <span className="text-2xl font-black text-accent">
                              {formatBrazilDate(
                                format(
                                  relationData.projectedContemplacaoDate,
                                  "yyyy-MM-dd",
                                ),
                              )}
                            </span>
                            <p className="text-[10px] font-bold text-muted-foreground opacity-60 mt-1">
                              Daqui a ~{relationData.mesesAteContemplacao} meses
                              de contribuição
                            </p>
                          </div>
                          <div className="pt-4 border-t border-accent/10 flex justify-between items-center">
                            <div>
                              <span className="text-[10px] font-black uppercase text-muted-foreground opacity-50 block">
                                Crédito Injetado
                              </span>
                              <span className="text-lg font-black text-primary">
                                {formatCurrency(relationData.liquidoUso)}
                              </span>
                            </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-5 rounded-2xl border border-border bg-muted/5 space-y-5">
                          <div>
                            ...
                            <span className="text-sm font-black text-muted-foreground opacity-60 line-through">
                              {formatCurrency(
                                relationData.saldoItauAtContemplacao,
                              )}
                            </span>
                          </div>
                          ...
                          <div className="pt-2">
                            ...
                            <span className="text-xl font-black text-green-600">
                              {formatCurrency(relationData.novoSaldoItau)}
                            </span>
                          </div>
                          ...
                          <div className="p-3 bg-green-500/5 rounded-xl border border-green-500/10 space-y-2">
                            <div className="flex justify-between items-center text-[10px] font-black uppercase">
                              ...
                              <span className="text-green-600">
                                -{relationData.economiaParcelas} meses
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[10px] font-black text-foreground">
                                {relationData.novasParcelasRestantes} (eram{" "}
                                {relationData.parcelasRestantesOriginal})
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="pt-2">
                            ...
                            <div className="flex items-center gap-2 text-primary">
                              ...
                              <span className="text-xl font-black">
                                {formatCurrency(relationData.economiaJuros)}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-5 rounded-2xl bg-muted/20 border border-border space-y-5 flex flex-col justify-between h-[300px]">
                          <div className="space-y-4">
                            <div>
                              <span className="text-[10px] font-black uppercase text-primary opacity-70 block mb-1">
                                Quitação Itaú
                              </span>
                              <span className="text-lg font-black text-foreground">
                                {formatBrazilDate(
                                  format(
                                    relationData.novaDataQuitacao,
                                    "yyyy-MM-dd",
                                  ),
                                )}
                              </span>
                              <p className="text-[9px] font-bold text-muted-foreground opacity-60">
                                Antecipação drástica do contrato SAC
                              </p>
                            </div>
                            <div>
                              <span className="text-[10px] font-black uppercase text-accent opacity-70 block mb-1">
                                Restante Consórcio
                              </span>
                              <span className="text-lg font-black text-foreground">
                                {relationData.consorcioRestante} parcelas
                              </span>
                              <p className="text-[9px] font-bold text-accent italic">
                                Total a pagar:{" "}
                                {formatCurrency(
                                  relationData.consorcioRestanteValor,
                                )}
                              </p>
                            </div>
                            <div className="pt-2 border-t border-border/30">
                              <span className="text-[10px] font-black uppercase text-muted-foreground opacity-50 block mb-1">
                                Sobra de Crédito (Cashback)
                              </span>
                              <span
                                className={`text-lg font-black ${relationData.sobraCredito >= 0 ? "text-green-600" : "text-destructive"}`}
                              >
                                {formatCurrency(relationData.sobraCredito)}
                              </span>
                              <p className="text-[9px] font-bold text-muted-foreground opacity-60">
                                Crédito Injetado - Quitação Itaú
                              </p>
                            </div>
                          </div>
                    ...
                    <div className="flex items-start gap-3 p-4 bg-primary/5 rounded-xl border border-primary/10">
                      ...
                      <p className="text-[10px] font-medium leading-relaxed text-muted-foreground opacity-70">
                        <span className="font-black text-primary uppercase mr-1">
                          Nota Técnica:
                        </span>
                        Esta simulação considera a manutenção do valor de
                        amortização base do Itaú após o aporte do consórcio. A
                        economia de juros é uma estimativa baseada no saldo
                        devedor amortizado e no custo de oportunidade do
                        capital. Valores reais podem variar conforme taxas de
                        seguros e correções monetárias (TR) não previstas nesta
                        projeção.
                      </p>
                    </div>
                  </>
                ...
                <table className="w-full text-left text-[10px] md:text-xs border-collapse">
                  <thead>
                    <tr className="bg-muted/50 text-foreground">
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider">
                        Parcela
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider">
                        Vencimento
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right">
                        Amort.
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right">
                        Juros
                      ...
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right text-blue-500">
                        FGTS Sub.
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right text-primary">
                        Total Pago
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right">
                        Saldo Dev.
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-center">
                        Observação
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-center">
                        Status
                    ...
                    {adjustedSacData.map((d: any) => (
                      ...
                      >
                        ...
                        <td className="p-3 text-muted-foreground">
                          {formatBrazilDate(d.vencimento)}
                        </td>
                        <td className="p-3 text-right font-mono">
                          {formatCurrency(d.amortizacao)}
                        </td>
                        <td className="p-3 text-right font-mono">
                          {formatCurrency(d.juros)}
                        </td>
                        <td className="p-3 text-right font-mono">
                          {formatCurrency(d.seguros)}
                        </td>
                        <td className="p-3 text-right font-mono text-blue-500">
                          {d.fgtsMensal > 0
                            ? `-${formatCurrency(d.fgtsMensal)}`
                            : "-"}
                        </td>
                ...
                <table className="w-full text-left text-[10px] md:text-xs border-collapse">
                  <thead>
                    <tr className="bg-muted/50 text-foreground">
                      ...
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right text-accent">
                        Valor (3 Cartas)
                      </th>
                      <th className="p-3 border-b border-border font-bold uppercase tracking-wider text-right">
                        Valor Individual
                  ...
                  <tbody className="divide-y divide-border/30">
                    {consorcioInstallmentsData.map((d: any) => (
                      ...
                      >
                        ...
                        <td className="p-3 text-right text-muted-foreground opacity-60">
                          {formatCurrency(d.valor / 3)}
                        </td>
                        <td className="p-3 text-right font-black text-primary">
                          {formatCurrency(d.faltaParaCredito)}
                        </td>
                        <td className="p-3 text-center">
                          <span
                            className={cn(
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 gap-6">
        {layout
          ...
          .map((item, index) => renderSection(item, index))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
          ...
          >
            ...
            <div className="flex-1 overflow-auto p-6">
              {renderSection(
                layout.find((i) => i.id === maximizedId)!,
                layout.findIndex((i) => i.id === maximizedId),
              )}
            </div>
          </div>
        </div>
