      ...
      tooltip: {
        callbacks: {
          label: function (context: TooltipItem<"doughnut">) {
            ...
            const percentage =
              categoryData.find((c) => c.category === label)?.percentage || 0;
        ...
        <Card className="p-6">
          ...
          <div className="h-64 mb-6 relative">
            ...
            {loadingAi && (
              <div className="absolute inset-0 bg-background/50 backdrop-blur-sm flex items-center justify-center rounded-2xl z-40">
                <p className="text-foreground text-lg font-black uppercase tracking-tighter animate-pulse">
          ...
          <div className="space-y-3">
            {categoryData.map((category, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-4 rounded-xl gap-3 border border-border bg-card/50 hover:bg-card transition-colors"
              >
          ...
          <Card key={index} className="p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm pt-2 border-t border-border">
                <span className="font-black text-foreground uppercase tracking-tighter truncate pr-2">
                  Saldo
                </span>
                <span
                  className={cn(
                    "font-black flex-shrink-0",
                    month.balance >= 0 ? "text-primary" : "text-accent",
                  )}
                >
