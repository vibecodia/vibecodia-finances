...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
      ...
      <div className="flex-1 min-w-[280px]">
        ...
        <div className="grid grid-cols-7 mb-2">
          {weekDays.map((day) => (
            <div
              key={day}
              className="text-center text-[10px] uppercase opacity-40 font-bold"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 gap-y-1">
          {days.map((day) => {
            ...
            if (selectingStart && hoverDate) {
              ...
              if (
                isWithinInterval(day, { start: hRangeStart, end: hRangeEnd })
              ) {
                isHoverInRange = true;
              }
            }
  ...
  const quickSelect = (
    type: "thisMonth" | "nextMonth" | "last7" | "last30",
  ) => {
        ...
        {([
          ...
          <button
            ...
            style={{
              ...
              borderColor: isActive(pill.value)
                ? theme.primary
                : theme.cardBorder,
