        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
              {format(days[0], "dd 'de' MMMM", { locale: ptBR })} -{" "}
              {format(days[days.length - 1], "dd 'de' MMMM", { locale: ptBR })}
            </p>
          </div>
        </div>
          ...
          <div className="relative">
            {tasksWithDates.length > 0 ? (
              tasksWithDates.map((task) => (
                <div
                  key={task.id}
                  className="flex border-b border-border/50 hover:bg-primary/5 transition-colors group"
                >
                  ...
                  <div className="flex-1 flex relative h-14">
                    ...
                    {days.map((day) => {
                      const dayLogs =
                        task.timeLogs?.filter((log) =>
                          isSameDay(new Date(log.date + "T12:00:00"), day),
                        ) || [];
