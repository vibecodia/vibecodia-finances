...
const actionConfig: Record<HistoryEntry['action'], { icon: React.ElementType; label: string; color: string }> = {
  create: { icon: Plus, label: 'Criação', color: 'text-green-500 bg-green-500/10' },
  update: { icon: Pencil, label: 'Atualização', color: 'text-blue-500 bg-blue-500/10' },
  move: { icon: ArrowRight, label: 'Movimentação', color: 'text-amber-500 bg-amber-500/10' },
  ...
  if (!history || history.length === 0) {
    return (
      <div className="py-8 text-center text-muted-foreground/40 border-2 border-dashed border-border rounded-xl">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest">Nenhum evento registrado</span>
      </div>
    );
  }
