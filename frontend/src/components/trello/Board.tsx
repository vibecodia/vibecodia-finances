...
export function Board() {
  ...
  const [columnViewModesByTheme, setColumnViewModesByTheme] = useLocalStorage<
    Record<string, Record<string, boolean>>
  >("trello_column_view_modes_by_theme", {});
  ...
  const toggleColumnMinimal = useCallback(
    (columnId: string) => {
  ...
  const [selectedFlagFilter, setSelectedFlagFilter] = useState<
    TaskFlag | "all"
  >("all");
  ...
  const finalFilteredTasks = useMemo(() => {
    return filteredTasks.filter((task) => {
      const flagMatch =
        selectedFlagFilter === "all" || task.flag === selectedFlagFilter;
      const labelMatch =
  ...
  const totalLoggedHours = useMemo(() => {
    return finalFilteredTasks.reduce((acc, task) => {
      const taskHours =
        task.timeLogs?.reduce((sum, log) => sum + log.hours, 0) || 0;
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (isMovingToDone && !wasInDone) {
      ...
      if (pendingDependencies && pendingDependencies.length > 0) {
        alert(
          "Esta tarefa não pode ser concluída pois possui dependências pendentes.",
        );
        return;
      }
    }
    ...
    if (destination.index >= visibleDestTasks.length) {
      ...
      const actualTargetIndex = newTasks.findIndex(
  ...
  const handleMoveForward = useCallback(
    (taskId: string) => {
      ...
      const currentColumnIndex = themeColumns.findIndex(
        (c) => c.id === task.columnId,
      );
      if (
        currentColumnIndex === -1 ||
        currentColumnIndex === themeColumns.length - 1
      )
        return;
    ...
    <div className="min-h-screen bg-background p-4 md:p-8 space-y-8 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onBlur={() => {
                      if (
                        tempThemeName.trim() &&
                        tempThemeName !== currentTheme.name
                      ) {
                        updateTheme(currentTheme.id, {
                          name: tempThemeName.trim(),
                        });
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-foreground/5 hover:bg-foreground/10 rounded-xl border border-border transition-all group/filter relative">
              ...
              <div className="relative flex items-center">
                ...
                >
                  ...
                  <option
                    value="impediment"
                    className="bg-background text-amber-500"
                  >
                    Impedimento
                  </option>
                  <option
                    value="paused"
                    className="bg-background text-blue-500"
                  >
                    Pausa
                  </option>
                </select>
            ...
            <button
              onClick={() =>
                setShowPendingChecklistOnly(!showPendingChecklistOnly)
              }
              className={cn(
      ...
      <div className="flex flex-wrap gap-4 overflow-x-auto pb-2">
        ...
        {themeColumns.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className={cn(
                "p-4 rounded-2xl border border-border flex flex-col gap-1 min-w-[140px]",
                color,
              )}
            >
              <span className="text-[10px] font-black uppercase tracking-widest opacity-70">
        ...
        <div className="p-4 rounded-2xl border border-border flex flex-col gap-1 bg-gray-500/10 text-gray-500 min-w-[140px]">
          ...
          <span className="text-2xl font-black">
            {finalFilteredTasks.filter((t) => t.columnId === "archived").length}
          </span>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-x-auto mt-8 pt-2 pb-8 custom-scrollbar [transform:rotateX(180deg)]">
            <div className="flex gap-8 h-full min-h-[600px] items-start [transform:rotateX(180deg)]">
              ...
              >
                ...
                <span className="text-xs font-black uppercase tracking-widest text-muted-foreground group-hover:text-primary">
                  Nova Coluna
                </span>
              </button>
            </div>
      ...
      <ConfirmationModal
        ...
        confirmText={
          columnDeleteConfirmation.hasTasks ? "Entendido" : "Confirmar Exclusão"
        }
      />
      ...
      <InputModal
        ...
        onClose={() =>
          setColumnInputModal((prev) => ({ ...prev, isOpen: false }))
        }
        ...
        message={
          importData ? (
            <div className="space-y-4 py-2">
              <p>
                Você está prestes a importar uma nova versão do seu quadro de
                tarefas.
              </p>
              <div className="bg-muted/50 p-4 rounded-xl border border-border space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-bold">
                    {new Date(importData.timestamp).toLocaleString("pt-BR")}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase opacity-50">
              ...
              <p className="text-destructive font-bold text-xs uppercase tracking-tight">
                Atenção: Isso substituirá todas as tarefas e temas atuais do seu
                quadro.
              </p>
            </div>
          ) : (
